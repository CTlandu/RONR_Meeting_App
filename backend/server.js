...
io.on("connection", (socket) => {
  ...
  if (!userConnections[userID]) {
    userConnections[userID] = { roomId: null, socketID: null, timer: null, username: null, role: null };
  }
  ...
  socket.on("joinRoom", ({ username, roomId }) => {
    ...
    if (!rooms[roomId][userID].reconnected) {
      socket.to(roomId).emit("userJoined", { username, userID, role: userConnections[userID].role });
  ...
  // Member 提交 Motion
socket.on("raiseMotion", ({ roomId, title, description, username }, callback) => {
  if (motions[roomId]) {
    // 如果当前房间有 motion 在处理, 拒绝新的 motion
    return callback({
      success: false,
      message: "There is already a motion in progress. Please wait until it is resolved.",
    });
  }
  ...
  socket.on("pendingMotionDecision", ({ roomId, motion, decision }) => {
    if (decision === "approved") {
      ...
      io.sockets.adapter.rooms.get(roomId).forEach((s) => {  // 通知除chair外所有人motion approved
      ...
      setTimeout(() => {
        if (motions[roomId].seconds === 0) {
          ...
          const discardMessage = motion.raisedBy + "'s motion: " + motion.title + ", was discarded as no one seconds.";
      ...
      const denyMessage = motion.raisedBy + "'s motion: " + motion.title + ", was denied by the chair.";
      ...
      if (userConnections[userID].socketID === currentSocketID) {
        ...
        if (chairs[roomId] === userID) {
          ...
          if (roomUsers.length > 0) {
            // 随机选择一个用户作为新的 Chair
            const newChair = roomUsers[Math.floor(Math.random() * roomUsers.length)];
            chairs[roomId] = newChair // 设置新的 Chair
            userConnections[newChair].role = "chair";
            rooms[roomId][newChair].role = "chair"; // 设置新的 Chair
            const newChairMessage = "The chair has been changed to: " + userConnections[newChair].username;
